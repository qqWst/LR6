...
void generateRandomKey(unsigned char* key) {
    ...
    for (int k = 0; k < 16; ++k) {
        key[k] = static_cast<unsigned char>(rand() % 256);
    }
}
...
void ExpandKey(unsigned char* master_key, unsigned char round_keys[11][4][4]) {
    ...
    for (int rnd = 1; rnd <= 10; ++rnd) {
        ...
        for (int r = 0; r < 4; ++r) {
            round_keys[rnd][r][0] = round_keys[rnd - 1][r][0] ^ tmp[r];
        }
...
void EncryptCFB(const vector<unsigned char>& textOrig,
    ...
    while (block_idx < textOrig.size()) {
        ...
        for (int i = 0; i < 16; ++i) {
            int r = i / 4;
            int c = i % 4;
            matrix[r][c] = feedback[i];
        }
        
        cout << "Matrix (State) до шифрования:\n";
        printMatrix(matrix, "  State");
        
        EncryptBlock(matrix, round_keys, true);
        
        cout << "Matrix (State) после шифрования:\n";
        printMatrix(matrix, "  State");
...
void GenerateIV(unsigned char* iv) {
    srand(static_cast<unsigned>(time(nullptr)));
    for (int k = 0; k < 16; ++k) {
        iv[k] = static_cast<unsigned char>(rand() % 256);
    }
}
...
int main() {
    ...
    cout << "\n\tПОЛНЫЙ ШИФРОТЕКСТ\n";
    cout << "Размер: " << textCript.size() << " байт\n";
    cout << "Данные: ";
    
    for (size_t i = 0; i < textCript.size(); ++i) {
        cout << hex << setw(2) << setfill('0') << static_cast<int>(textCript[i]);
        if ((i + 1) % 32 == 0 && (i + 1) < textCript.size()) {
            cout << "\n        ";
        }
    }
    cout << dec << "\n\n";
